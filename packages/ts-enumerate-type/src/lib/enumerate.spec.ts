...
import { Enumerate, EnumerateExclusive, EnumerateInclusive } from "../types/Enumerate";

import { isInEnumeration, isInExclusiveEnumeration, isInInclusiveEnumeration } from "./enumerate";
...
test("strongly typed array access", (t) => {
  ...
  const getItemInArray = (arr: FixedSizeArray<any, 3>, index: EnumerateExclusive<3>) => {
    return arr[index];
  };
